...
```text
分析项目 -> 检索源码 -> 生成候选补丁 -> 校验补丁 -> 人工确认 -> 应用修改 -> 构建测试
```

输入开发任务并配置 model、API Key 和 Base URL 后，Coding Agent 会生成修改计划、风险、建议测试和标准 unified diff。候选补丁不会自动写入项目，只有点击“应用补丁”后才会修改文件。
...
当前状态：已完成受控修改 MVP。下一步将加入基于编译错误的自动修复循环、按文件接受变更和 Git branch/commit 工作流。
